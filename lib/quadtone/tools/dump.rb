...
module Quadtone
  ...
  class DumpTool < Tool
    ...
    def run(args)
      ...
      curveset.write_svg_file(tmp_dir + path.with_extname('1.svg').basename, :normalize => false)
      ;;curveset.dump

      curveset.curves[0] = curveset.curves.first.resample(11)
      curveset.write_svg_file(tmp_dir + path.with_extname('2.svg').basename, :normalize => false)
      ;;curveset.dump
